...
const TOOLS = [
  ...
  { id: "text", icon: TypeIcon, label: "Label" },
...
export default function Workspace() {
  ...
  const send = () => {
    // Trim + cap length: keep AI-terminal messages from growing unbounded
    // and avoid submitting whitespace-only prompts.
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-background text-foreground overflow-hidden">
      ...
      <div className="flex-1 flex min-h-0">
        <div className="w-14 border-r border-border flex flex-col items-center py-4 gap-1 shrink-0">
          {TOOLS.map((t) => (
            <button
              ...
              title={t.label}
              className={cn(
                "h-10 w-10 grid place-items-center rounded-lg transition",
                tool === t.id ? "bg-primary/15 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-surface",
              )}
            >
        ...
        <div className="flex-1 relative dot-bg overflow-hidden">
          <div
            className="absolute inset-0"
            style={{ background: "radial-gradient(ellipse at 30% 20%, oklch(0.78 0.14 225 / 0.08), transparent 60%)" }}
          />
          <svg className="absolute inset-0 w-full h-full pointer-events-none" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <marker id="arrow" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
                <path d="M0,0 L10,5 L0,10 z" fill="oklch(0.78 0.14 225 / 0.7)" />
              </marker>
            </defs>
            <path d="M 260 220 C 340 220, 340 340, 420 340" stroke="oklch(0.78 0.14 225 / 0.7)" strokeWidth="1.5" fill="none" markerEnd="url(#arrow)" />
            <path
              d="M 260 260 C 340 260, 340 460, 420 460"
              stroke="oklch(0.78 0.14 225 / 0.5)"
              strokeWidth="1.5"
              fill="none"
              markerEnd="url(#arrow)"
              strokeDasharray="4 4"
            />
            <path d="M 580 340 C 660 340, 660 460, 740 460" stroke="oklch(0.78 0.14 225 / 0.6)" strokeWidth="1.5" fill="none" markerEnd="url(#arrow)" />
          </svg>

          <Node style={{ left: 120, top: 200 }} title="API Gateway" tag="edge" />
          <Node style={{ left: 420, top: 320 }} title="Checkout Svc" tag="core" accent />
          <Node style={{ left: 420, top: 440 }} title="Payments Svc" tag="core" accent />
          <Node style={{ left: 740, top: 440 }} title="Kafka · orders.v1" tag="stream" />
          <Node style={{ left: 740, top: 300 }} title="Postgres · checkout" tag="store" />

          <div className="absolute bottom-4 left-4 flex items-center gap-2 rounded-full bg-surface/80 backdrop-blur border border-border px-3 py-1.5 text-xs font-mono text-muted-foreground">
            <span>100%</span>
            <span className="text-border">|</span>
            <span>x: 420  y: 340</span>
          </div>
        </div>
        ...
        >
          <div className="flex items-center justify-between px-4 pt-4">
            ...
            <button className="text-muted-foreground hover:text-foreground lg:hidden" onClick={() => setPanelOpen(false)}>
              <X className="h-4 w-4" />
            </button>
          </div>
...
function Node({ style, title, tag, accent }) {
  return (
    <div
      className={cn(
        "absolute w-40 rounded-xl border bg-card/90 backdrop-blur px-3 py-2.5 shadow-lg",
        accent ? "border-primary/50 ring-1 ring-primary/30" : "border-border",
      )}
      style={style}
    >
      <div className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">{tag}</div>
      <div className="text-sm font-semibold mt-0.5">{title}</div>
      <div className="mt-2 flex items-center gap-1">
        <span className="h-1.5 w-1.5 rounded-full bg-[color:var(--success)]" />
        <span className="text-[10px] font-mono text-muted-foreground">healthy · 3 replicas</span>
      </div>
    </div>
  );
}
...
function Bubble({ author, role, text }) {
  ...
  return (
    <div className={cn("flex gap-2", isSelf && "flex-row-reverse")}>
      ...
      >
        {/* text is rendered as plain text (not HTML), so it can't inject markup */}

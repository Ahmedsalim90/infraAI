...
const ALLOWED_EXTENSIONS = [".json", ".yaml", ".yml", ".tf", ".mmd", ".md"];
...
export function ImportModal({ open, onClose, onImport, title, description }) {
  ...
  const onFileChange = (e) => {
    ...
    // Client-side checks are UX only — the upload endpoint must re-validate
    // file type, size and contents (and scan it) server-side before trusting it.
    ...
    <Modal open={open} onClose={onClose} title={title} description={description}>
      <label className="block cursor-pointer">
        ...
        <div className="rounded-xl border-2 border-dashed border-border bg-surface/50 p-8 text-center hover:border-primary/50 transition">
          ...
          <div className="text-xs text-muted-foreground mt-1">JSON · YAML · Terraform · Mermaid — up to 10 MB</div>
        </div>
      </label>
...
export function ExportModal({ open, onClose, onExport }) {
  ...
  const options = ["JSON", "YAML", "Terraform", "Mermaid diagram", "PDF report"];
  return (
    <Modal open={open} onClose={onClose} title="Export" description="Choose a format. Files are generated locally in the browser.">
